...
const BattleComponent = ({ songDataToSort }: { songDataToSort: string[] }) => {
        ...
        try {
            ...
            for (let i = 0; i < sortedIndexList.current.length; i++) {
                if (sortedIndexList.current[i].length > 1) {
                    ...
                    sortedIndexList.current[marker] = parent.slice(midpoint, parent.length)
                    totalBattles.current = totalBattles.current + (sortedIndexList.current[marker]).length
                    parentIndexList.current[marker] = i
                    marker++
                }
    ...
    const updateSongs = () => {
        const leftSongIndex = sortedIndexList.current[leftIndex.current][leftInnerIndex.current]
        const rightSongIndex = sortedIndexList.current[rightIndex.current][rightInnerIndex.current]

        setLeftSong(songDataToSort[leftSongIndex])
        setRightSong(songDataToSort[rightSongIndex])

        setProgress(Math.floor((sortedNumber.current * 100) / (totalBattles.current/2)))
    }
    ...
    const handleSelect = (choice: "left" | "right" | "tie") => {
        ...
        if (leftInnerIndex.current === leftListLen && rightInnerIndex.current === rightListLen) {
            ...
            (sortedIndexList.current).forEach((_: any, idx: number) => recordDataList.current[idx] = 0)
            pointer.current = 0
        }
        if (leftIndex.current < 0) {
            ...
            songDataToSort.forEach((_, idx) => {
                ...
                if (idx < songDataToSort.length - 1) {
                    if (tiedDataList.current[songIndex] === finalSortedIndexes[idx + 1]) {
                        tiedRankNum++
                    } else {
                        rankNum += tiedRankNum
                        tiedRankNum = 1
    ...
    if (showResults) {
        return <Results finalResults={finalSongs.current || []} history={history.current} songDataToSort={songDataToSort}/>
    }
    ...
    return (
        <div style={{ textAlign: "center", fontFamily: "Arial, sans-serif" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "10px", margin: "20px 0" }}>
                <img src={leftSong} alt="Left Album" style={{ height: "150px", width: "150px" }} />
                ...
                <img src={rightSong} alt="Right Album" style={{ height: "150px", width: "150px" }} />
            </div>
